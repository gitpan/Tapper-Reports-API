...
#     BUILD_REQUIRES => { Test::Fixture::DBIC::Schema=>q[0], Test::Perl::Critic=>q[0], Test::More=>q[0], Tapper::Test=>q[3.000010], ExtUtils::MakeMaker=>q[6.42], Test::Deep=>q[0], Cwd=>q[0] }
...
#     NO_META => q[1]
#     PREREQ_PM => { Test::Fixture::DBIC::Schema=>q[0], ExtUtils::MakeMaker=>q[6.42], Tapper::Test=>q[3.000010], Net::Server=>q[0], Tapper::Schema=>q[3.000010], Test::More=>q[0], Test::Perl::Critic=>q[0], Tapper::Config=>q[3.000010], Tapper::Reports::DPath=>q[3.000010], Data::Structure::Util=>q[0], Module::Build=>q[0], Test::Deep=>q[0], MooseX::Daemonize=>q[0.08], Cwd=>q[0], Moose=>q[0.72] }
#     VERSION => q[3.000010]
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Tapper/Reports/API.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Tapper/Reports
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Tapper/Reports
...
TO_INST_PM = lib/Tapper/Reports/API.pm \
	lib/Tapper/Reports/API/Daemon.pm

PM_TO_BLIB = lib/Tapper/Reports/API/Daemon.pm \
	blib/lib/Tapper/Reports/API/Daemon.pm \
	lib/Tapper/Reports/API.pm \
	blib/lib/Tapper/Reports/API.pm
...
DISTVNAME = Tapper-Reports-API-3.000010
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Tapper/Reports/API.pm $(INST_MAN3DIR)/Tapper::Reports::API.$(MAN3EXT) 
...
EXE_FILES = bin/tapper-reports-api bin/tapper-reports-api-daemon

pure_all :: $(INST_SCRIPT)/tapper-reports-api-daemon $(INST_SCRIPT)/tapper-reports-api
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/tapper-reports-api-daemon $(INST_SCRIPT)/tapper-reports-api 

$(INST_SCRIPT)/tapper-reports-api-daemon : bin/tapper-reports-api-daemon $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/tapper-reports-api-daemon
	$(CP) bin/tapper-reports-api-daemon $(INST_SCRIPT)/tapper-reports-api-daemon
	$(FIXIN) $(INST_SCRIPT)/tapper-reports-api-daemon
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/tapper-reports-api-daemon

$(INST_SCRIPT)/tapper-reports-api : bin/tapper-reports-api $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/tapper-reports-api
	$(CP) bin/tapper-reports-api $(INST_SCRIPT)/tapper-reports-api
	$(FIXIN) $(INST_SCRIPT)/tapper-reports-api
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/tapper-reports-api
...
ppd :
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Tapper::Config" VERSION="3.00001" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Tapper::Reports::DPath" VERSION="3.00001" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Tapper::Schema" VERSION="3.00001" />' >> $(DISTNAME).ppd
